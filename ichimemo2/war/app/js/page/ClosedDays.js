var ClosedDays = Page.def(function ClosedDays(){}, function(Class){

function ClosedDays(xpath, pleceMsg){
	this.xpath = xpath;
	var days = Class.BASE_DATA;
	var html = "";
	for (var i=0; i<days.length; i++) {
		var name1 = days[i]+"-L";
		var name2 = days[i]+"-D";
		var label = Class.MAPPING[days[i]]+"曜日";
		if (days[i] == "Fet") {
			html += "<li></li>"
			label = Class.MAPPING[days[i]]+"日";
		}
...
Class.init = function() {
	Class.INSTANCE = new ClosedDays("#spotDaysList");
}
Class.open = function() {
...
Class.setValue = function(list) {
	...
	for (var i=0; i<list.length; i++) {
		var item = list[i];
		var day = Class.MAPPING[item.substr(0,1)];
		if (item.length == 1) {
...
Class.getValue = function() {
	...
	for (var i=0; i<days.length; i++) {
		...
		var youbi = Class.MAPPING[days[i]];
		if (isL && isD) {
			list.push(youbi);
		} else if (isL && !isD) {
			list.push(youbi+"昼");
